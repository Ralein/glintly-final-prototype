...
function GlintlySignupPage() {
  ...
  const isPopupSupported = () => {
    try {
      return !!(window && window.open() && !window.navigator.userAgent.includes('Instagram'));
    } catch {
      return false;
    }
  };
    ...
    try {
      ...
      if (isPopupSupported()) {
        try {
          ...
          // If popup fails (except for user-cancelled), try redirect
          if (popupError.code !== 'auth/popup-closed-by-user' && 
              popupError.code !== 'auth/cancelled-popup-request') {
            await signInWithRedirect(auth, provider);
            return; // Exit here as redirect will handle the rest
          } else {
            throw popupError; // Re-throw user cancellation errors
          }
        }
      } else {
        // Use redirect for environments that don't support popups
        await signInWithRedirect(auth, provider);
        return;
      }

      // If we got a result from popup, navigate
      if (result && result.user) {
        router.push("/profile-setup");
      }
      ...
      // Only show error for non-user-cancelled errors
      if (error.code !== 'auth/popup-closed-by-user' && 
            ...
            <div className="relative bg-gradient-to-br from-white/10 via-white/5 to-white/10 backdrop-blur-xl rounded-3xl border border-white/20 p-8 shadow-2xl">
              ...
              <div className="text-center mb-8">
                ...
                <p className="text-gray-300 text-lg">
                  Start your learning journey with Glintly
                </p>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <Button
                  ...
                  disabled={isLoading || isGoogleLoading || !email || !password || !confirmPassword || !firstName || !lastName}
                >
...
export default GlintlySignupPage;